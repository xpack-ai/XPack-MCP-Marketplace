...
export const EmailConfigForm: React.FC<EmailConfigFormProps> = ({
  ...
  return (
    ...
    }} defaultExpandedKeys={['email-config']} className='px-0'>
      <AccordionItem key="email-config" title={
        <div className="flex flex-col justify-between">
          <h3 className="text-lg font-semibold flex items-center gap-2">
            {t('Email Configuration')}
          </h3>
          <p className="text-sm text-gray-500">{t('Configure SMTP settings for email notifications')}</p>
        </div>
      }>
        <div className="space-y-4">
          ...
          <div className={`flex gap-3 ${isCustomPort ? 'flex-col' : ''}`}>
            <div className="flex-1">
              <Select
                ...
                selectedKeys={isCustomPort ? ["custom"] : (formData.smtp_port ? [formData.smtp_port] : [])}
              ...
              >
                {commonPorts.map((port) => (
                  <SelectItem key={port.key} value={port.key} textValue={port.label}>
                    {port.label}
                  </SelectItem>
                ))}
              </Select>
            ...
            {isCustomPort && (
              <div className="flex-1">
                <Input
                  ...
                  min="1"
                  max="65535"
                  color={formData.smtp_port && !isValidPort(formData.smtp_port) ? "danger" : "default"}
                  errorMessage={formData.smtp_port && !isValidPort(formData.smtp_port) ? t('Invalid port number') : ""}
                  isRequired
                />
              </div>
            )}
          ...
          <Input
            ...
            type="email"
            isRequired
            color={formData.smtp_sender && !isValidEmail(formData.smtp_sender) ? "danger" : "default"}
            errorMessage={formData.smtp_sender && !isValidEmail(formData.smtp_sender) ? t('Invalid email format') : ""}
          />
        </div>
